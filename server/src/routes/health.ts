// GET /healthz — база, Redis, хранилище (раздел 10.2 server-spec.md). Любой отказ — 503 с
// подробностями, чтобы мониторинг и бот админа видели, что именно упало.
...
async function run(fn: () => Promise<void>): Promise<Check> {
...
export async function healthRoutes(app: FastifyInstance, services: Services): Promise<void> {
  app.get('/healthz', async (_req, reply) => {
    const [db, redis, storage] = await Promise.all([
      ...
      run(() => checkRedis(services.redis)),
      run(() => checkStorage(services.storage))
    ]);
    const checks = { db, redis, storage };
    const ok = db.ok && redis.ok && storage.ok;
    reply.code(ok ? 200 : 503);
    return { ok, checks, version: services.version, uptime: Math.round(process.uptime()) };
  });
}
